...
class SummaryMixing(nn.Module):
    ...
    def __init__(
        ...
        if (
            self.mode == "SummaryMixing"
            or self.mode == "SummaryMixing-expdecay"
        ):
    ...
    def forward(self, x, sum_mask=None, src_padding_mask=None):
        ...
        if src_padding_mask is not None:
            src_padding_mask = (
                torch.logical_not(src_padding_mask).unsqueeze(-1).float()
            )
        else:
            src_padding_mask = (
                torch.ones((x.shape[0], x.shape[1])).unsqueeze(-1).float()
            )

        if sum_mask is not None:
            sum_mask = torch.logical_not(sum_mask).float()
    ...
    def _forward_mixing(self, x, sum_mask, src_padding_mask):
        ...
        if self.mode == "SummaryMixing-expdecay":
            sum_mask = self._laplace_weights(
                T, self.decay_constant, sum_mask, x.device
            )
        ...
        return self.summary_local_merging(
            self.time_dropout(torch.cat([local_summary, time_summary], dim=-1))
        )
    ...
    def _forward_mixing_fast(self, x, sum_mask, src_padding_mask):
        ...
        split_global_proj = torch.split(
            global_proj, self.local_proj_out_dim, dim=-1
        )
        ...
        if sum_mask is None:
            ...
            time_summary = (
                torch.matmul(split_global_proj[1].mT, sum_mask.T).mT
                / full_mask_with_pad
            )

        return self.dropout(
            self.summary_local_merging(
                torch.cat([split_global_proj[0], time_summary], dim=-1)
            )
        )
